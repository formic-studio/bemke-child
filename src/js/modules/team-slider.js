...
function hideVisualSlideNumber(slide) {
  const visualNumber = Array.from(slide.children).find((element) =>
    /^0*\d+$/u.test(element.textContent?.trim() ?? ''),
  );

  visualNumber?.setAttribute('aria-hidden', 'true');
}
