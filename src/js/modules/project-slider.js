...
const SLIDE_SELECTOR = '.slide-project';
const CONTROLS_SELECTOR = '.slider-paggination';
const ARROW_SELECTOR = '.yellow-arrow, .arrow';
const FOCUSABLE_SELECTOR =
  'a[href], button:not([disabled]), input:not([disabled]), select:not([disabled]), textarea:not([disabled]), [tabindex]';

const READY_ATTR = 'data-bemke-project-slider-ready';
const ACTIVE_ATTR = 'slide-active';
const BOOT_FLAG = '__bemkeProjectSliderBooted';
...
export function initProjectSlider() {
  ...
  setupProjectSliderLifecycle();
}

function initProjectSliderRoots(scope = document) {
  scope.querySelectorAll(ROOT_SELECTOR).forEach((root) => {
    if (root.getAttribute(READY_ATTR) === '1') {
      root.__bemkeProjectSliderRefresh?.();
      return;
    }

    createProjectSlider(root);
  });
}

function setupProjectSliderLifecycle() {
  if (window[BOOT_FLAG]) {
    return;
  }
...
function createProjectSlider(root) {
  const track = root.querySelector(TRACK_SELECTOR);
  const slides = track
    ? Array.from(track.querySelectorAll(`:scope > ${SLIDE_SELECTOR}:not(.${GHOST_CLASS})`))
    : [];
...
function decorateSlider(root, track, slides) {
  ...
  if (!track.id) {
    track.id = `bemke-project-slider-track-${sliderId}`;
  }
...
function createGhostSlide(sourceSlide) {
  ...
  ghost.querySelectorAll(FOCUSABLE_SELECTOR).forEach((node) => {
...
function rememberFocusableState(slide) {
  slide.querySelectorAll(FOCUSABLE_SELECTOR).forEach((node) => {
    ...
    node.setAttribute(ORIGINAL_TABINDEX_ATTR, node.getAttribute('tabindex') ?? '');
  });
}
