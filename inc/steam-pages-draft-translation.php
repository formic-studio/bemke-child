<?php
/** Reviewed English drafts for three STEAM workshop pages. */

if ( ! defined( 'ABSPATH' ) ) {
	exit;
}

add_action( 'admin_menu', 'bemke_child_register_steam_pages_translation_page' );
add_action( 'admin_post_bemke_create_steam_pages_en_drafts', 'bemke_child_create_steam_pages_en_drafts' );

function bemke_child_register_steam_pages_translation_page() {
	add_management_page( 'Bemke EN — warsztaty STEAM', 'Bemke EN — warsztaty STEAM', 'manage_options', 'bemke-steam-pages-translation', 'bemke_child_render_steam_pages_translation_page' );
}

/** @return array<string, mixed>|WP_Error */
function bemke_child_get_steam_pages_translation_batch() {
	$path = get_stylesheet_directory() . '/data/steam-pages-bemke-en.json';
	if ( ! is_readable( $path ) ) {
		return new WP_Error( 'missing_batch', 'Brakuje planu partii warsztatów STEAM.' );
	}
	...
	$ids   = array( 1056, 1052, 1054 );
	if ( ! is_array( $batch ) || 'steam-pages-4' !== ( $batch['batch'] ?? null ) || ! isset( $batch['plans'] ) || ! is_array( $batch['plans'] ) || count( $batch['plans'] ) !== count( $ids ) ) {
		return new WP_Error( 'invalid_batch', 'Plan partii warsztatów STEAM ma nieprawidłowy format.' );
	}
	foreach ( $batch['plans'] as $index => $plan ) {
		if ( ! is_array( $plan ) || $ids[ $index ] !== ( $plan['source_post_id'] ?? null ) || ! isset( $plan['source_post_parent'], $plan['source_post_title'], $plan['source_excerpt'], $plan['source_bricks_sha256'], $plan['source_bricks_elements'], $plan['target_title'], $plan['target_slug'], $plan['target_excerpt'], $plan['bricks_edits'], $plan['yoast_edits'] ) || ! is_array( $plan['bricks_edits'] ) || ! is_array( $plan['yoast_edits'] ) ) {
			return new WP_Error( 'invalid_batch', 'Nieprawidłowe dane strony w partii warsztatów STEAM.' );
		}
	}
	return $batch;
}
...
function bemke_child_validate_steam_pages_translation( $batch ) {
	...
	foreach ( $batch['plans'] as $plan ) {
		...
		$orphans = get_posts( array( 'post_type' => 'page', 'post_status' => array( 'draft', 'pending', 'private', 'publish', 'future' ), 'meta_key' => '_bemke_steam_pages_en_source_id', 'meta_value' => (string) $id, 'posts_per_page' => 1, 'suppress_filters' => true ) );
		if ( ! $existing && $orphans ) {
...
function bemke_child_render_steam_pages_translation_page() {
	if ( ! current_user_can( 'manage_options' ) ) {
		wp_die( esc_html__( 'Brak dostępu.', 'bemke-child' ) );
	}
	$batch = bemke_child_get_steam_pages_translation_batch();
	echo '<div class="wrap"><h1>Bemke EN — warsztaty STEAM</h1>';
	if ( is_wp_error( $batch ) ) {
		echo '<div class="notice notice-error"><p>' . esc_html( $batch->get_error_message() ) . '</p></div></div>';
		return;
	}
	$state = bemke_child_validate_steam_pages_translation( $batch );
	if ( isset( $_GET['created'] ) ) {
		echo '<div class="notice notice-success"><p>Utworzono trzy szkice EN. Strony PL oraz publiczne menu pozostają bez zmian.</p></div>';
	}
	if ( $state['errors'] ) {
	...
	echo '<p>Narzędzie tworzy trzy powiązane szkice EN z tekstami Bricks i polami Yoast. Angielskie menu można potem uzupełnić przez Narzędzia → Bemke EN — nawigacja. Opisy ALT EN uzupełnimy w osobnym etapie.</p>';
	...
	if ( ! $state['errors'] && ! $state['orphans'] && ! $state['existing'] ) {
		echo '<form method="post" action="' . esc_url( admin_url( 'admin-post.php' ) ) . '"><input type="hidden" name="action" value="bemke_create_steam_pages_en_drafts">';
		wp_nonce_field( 'bemke_create_steam_pages_en_drafts' );
		submit_button( 'Utwórz 3 szkice EN — warsztaty STEAM' );
		echo '</form>';
	}
	echo '<p>Dokumenty PDF nadal są po polsku. Część przycisków pobierania jest oznaczona „in Polish”. Przed publikacją sprawdź terminy rekrutacji, linki oraz podgląd każdej strony. Na stronie warsztatów cyklicznych dwa opisy kart już w wersji PL nie pasują do ich tytułów; szkic EN zachowuje treść źródłową do korekty redakcyjnej.</p>';
	foreach ( $batch['plans'] as $plan ) {
...
function bemke_child_create_steam_pages_en_drafts() {
	if ( ! current_user_can( 'manage_options' ) ) {
		wp_die( esc_html__( 'Brak dostępu.', 'bemke-child' ) );
	}
	check_admin_referer( 'bemke_create_steam_pages_en_drafts' );
	$batch = bemke_child_get_steam_pages_translation_batch();
	if ( is_wp_error( $batch ) ) {
	...
	$state = bemke_child_validate_steam_pages_translation( $batch );
	if ( $state['errors'] || $state['orphans'] || $state['existing'] ) {
		wp_die( esc_html( 'Plan jest nieaktualny lub szkice EN już istnieją. Sprawdź Narzędzia → Bemke EN — warsztaty STEAM.' ) );
	}
	$created = array();
	foreach ( $batch['plans'] as $plan ) {
		...
		update_post_meta( $post_id, '_bemke_steam_pages_en_source_id', $source_id );
	...
	$linked = array();
	foreach ( array( 7, 173, 275, 317, 341, 376, 378, 380, 445, 460, 545, 618, 645, 659, 739, 857, 924, 981, 996, 1052, 1054, 1056, 3345 ) as $source_id ) {
	...
	wp_safe_redirect( admin_url( 'tools.php?page=bemke-steam-pages-translation&created=3' ) );
	exit;
}
